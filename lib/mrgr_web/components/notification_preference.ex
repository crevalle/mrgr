defmodule MrgrWeb.Components.NotificationPreference do
  ...
  def preference_form(%{preference: %{event: @big_pr}} = assigns) do
    ~H"""
    <%= form_for @changeset, "#", [class: "notification-preference-form", phx_change: "preference-updated", phx_target: @target], fn f -> %>
  ...
  def preference_form(%{preference: %{event: @pr_controversy}} = assigns) do
    ~H"""
    <%= form_for @changeset, "#", [class: "notification-preference-form", phx_change: "preference-updated", phx_target: @target], fn f -> %>
      <%= label(f, :thread_length_threshold, "Thread Length Threshold",
        class: "text-sm font-medium text-gray-700"
      ) %>
      <.input field={f[:thread_length_threshold]} type="number" min="1" class="w-24 p-1" />
    <% end %>
    """
  end
